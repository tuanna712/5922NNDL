...
class PossitionalEmbedding(nn.Module):
    def __init__(self, d_model:int, seq_len:int, dropout:float)-> None:
        ...
        possition = torch.arange(0, seq_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, d_model, 2).float()) * (-math.log(10000.0)/d_model)
        # Apply the sin to event positions
...
class Transformer(nn.Module):
    def __init__(self, encoder: Encoder, decoder: Decoder, 
                 ...
                 scr_pos: PossitionalEmbedding, tgt_pos: PossitionalEmbedding,
...
def build_transformer(src_vocab_size:int, tgt_vocab_size:int, 
                      ...
                      N:int=6, h:int=8, dropout:float=.1) -> Transformer:
    # Create the embedding layers
    src_embed = InputEmbedding(d_model, src_vocab_size)
    tgt_embed = InputEmbedding(d_model, tgt_vocab_size)
    
    # Create the positional embedding layers
    src_pos = PossitionalEmbedding(d_model, src_seq_len, dropout)
    tgt_pos = PossitionalEmbedding(d_model, tgt_seq_len, dropout)
